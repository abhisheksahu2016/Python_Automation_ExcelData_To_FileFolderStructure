...
Excel_Path = ("S:/2_PCRC/C22_PyDj_Sb_An/A32_ExcelToProject/2_Final/1_In/ExcelFileFolderStruc.xlsx")
...
Folder_Structure = Excel_Open.sheet_by_index(2) 
...
path = "S:/2_PCRC/C22_PyDj_Sb_An/A32_ExcelToProject/2_Final/3_Out/"
temp_path=""
for i in range(len(Folder_Structure_NumpyMatrix)):
        ...
        for j in range(len(Folder_Structure_NumpyMatrix[i])):
                if Folder_Structure_NumpyMatrix[i][j] !="NIL" :
                    # ------------------
                    File_Extension = (".txt", ".pdf", ".docx" ,".pptx",".psd",".png","xlsx",".lnk",".skp",".html",".css",".sass",".js",".json",".csv",".cs",".cshtml",".cshtml.cs")
                    ...
                    if Chk_CellValueType is True :
                        ...
                        if Chk_Shortcut is True : 
                            if 'V-0.0.0.txt.lnk' in temp_str:
                                Src_Path = 'F:/1.Fuschia_1.0.8/3.Pro_Data/2.Fuschia_DTsn/2.Kernal_Space/1-3.Application_Memory/3.Pro_Data/2.xxx_DTsn/2.Data/1.Shaktiman/3.Pro_Data/2.ShMn_Datation/New folder/3.Information/21.Brough/1.xxx_V-0.0.0.txt.lnk'
                            ...
                            if Chk_File is True :
                                ...
                                if Chk_File is True :
                                    ...
                                    """
                                    # -------Single ----------
                                    SrcPath = 'C:/Users/ABHISHEK/Desktop/try-1.txt'
                                    DstPath = 'C:/Users/ABHISHEK/Desktop/try-2.txt'
                                    #-------------------------------------------
                                    TempList =SrcPath.split('/') 
                                    SrcFile = str(TempList[len(TempList)-1])
                                    print(SrcFile)  
                                    #-------------------------------------------
                                    Chk_File = os.path.exists(SrcPath) 
                                    print(Chk_File)
                                    if Chk_File is True :
                                        try : 
                                            os.rename(SrcPath, DstPath) 
                                            print("Source path renamed to DstPathination path successfully.") 
                                        except IsADirectoryError: 
                                            print("Source is a file but DstPathination is a directory.") 
                                        except NotADirectoryError: 
                                            print("Source is a directory but DstPathination is a file.")  
